...
| **Flashblocks** | feature | Code name for the sub-second pre-confirmation block streaming feature. On OP Stack chains it is surfaced as **Flashblocks** (legacy name, the default) or **Subblocks** (OP Labs' newer name) per `NEXT_PUBLIC_FLASHBLOCKS_NAME`, since chains are split between the two while sharing one wire protocol; on MegaETH it is surfaced as **mini-blocks**. The `flashblocks` code name and `FLASHBLOCKS` env vars are retained. |
