...
| NEXT_PUBLIC_FLASHBLOCKS_NAME | `'flashblock' \| 'subblock'` | What the chain calls its pre-confirmation blocks. Drives every user-facing label and the blocks page tab id (`flashblocks` or `subblocks`); the other tab id keeps working and redirects to the configured one. | - | `flashblock` | `subblock` | upcoming |
